...
#define kBaseViewHeight (self.maxPreviewCount ? 322 : 142)
...
@property (nonatomic, strong) NSMutableArray *showCellArr;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.bottomConstants.constant = L2C_TabbarSafeBottomMargin;
        ...
        self.showCellArr = [NSMutableArray array];
    }
    return self;
}
...
//    [self.sender presentVC:alert animated:YES completion:nil];
}
...
- (void)show
{
    ...
    if (self.animate) {
        ...
        CGPoint toPoint   = CGPointMake(kViewWidth/2, kViewHeight-(kBaseViewHeight)/2 - L2C_TabbarSafeBottomMargin);
...
//            [self.sender presentVC:picker animated:YES completion:nil];
...
- (IBAction)btnPhotoLibrary_Click:(id)sender
{
    if (![self judgeIsHavePhotoAblumAuthority]) {
        ...
        photoBrowser.isSelectOriginalPhoto = self.isSelectOriginalPhoto;
...
- (IBAction)btnOriginalClick:(id)sender {
    self.isSelectOriginalPhoto = !self.originalBtn.selected;
...
- (void)getOriginalImageBytes
{
    ...
    if (self.isSelectOriginalPhoto && self.arraySelectPhotos.count > 0) {
        [[ZLPhotoTool sharePhotoTool] getPhotosBytesWithArray:self.arraySelectPhotos completion:^(NSString *photosBytes) {
                    ...
                    self.originalBtn.selected = self.isSelectOriginalPhoto;
                } else {
                    self.originalBtn.selected = NO;
                    self.labPhotosBytes.text = @"";
                }
    }
//    if (self.isSelectOriginalPhoto && self.arraySelectPhotos.count > 0) {
//        [[ZLPhotoTool sharePhotoTool] getPhotosBytesWithArray:self.arraySelectPhotos completion:^(NSString *photosBytes) {
...
- (void)cell_btn_Click:(UIButton *)btn
{
    ...
    ZLCollectionCell *cell = (ZLCollectionCell *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:btn.tag inSection:0]];
    if (!btn.selected) {
    ...
    if(btn.selected == YES){
        ...
        [btn setImage:[UIImage imageNamed:@"btn_selected@3x.png"] forState:UIControlStateNormal];

    }else{
        
        [btn setImage:[UIImage imageNamed:@"btn_circle@3x.png"] forState:UIControlStateNormal];
    }
...
- (void)changeBtnCameraTitle
{
    if (self.arraySelectPhotos.count > 0) {
        ...
        self.btnCamera.userInteractionEnabled = YES;
        ...
        [self.btnCamera setBackgroundImage:[UIImage imageNamed:@"yj_icon_wanchengxuanzhong"] forState:(UIControlStateNormal)];
//        [self.btnCamera setTitleColor:kRGB(19, 153, 231) forState:UIControlStateNormal];
    } else {
//        [self.btnCamera setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserCameraText) forState:UIControlStateNormal];
        self.originalBtn.enabled = NO;
        [self.btnCamera setTitle:[NSString stringWithFormat:@"%@", GetLocalLanguageTextValue(ZLPhotoBrowserDoneText)] forState:UIControlStateNormal];
        [self.btnCamera setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
         [self.btnCamera setBackgroundImage:[UIImage imageNamed:@"yj_icon_wanchengmeixuan"] forState:(UIControlStateNormal)];
        
        self.btnCamera.userInteractionEnabled = NO;
    }
    [self getOriginalImageBytes];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSelector:@selector(setCellBtnPosition:) withObject:indexPath afterDelay:0.03];
    
    if(cell.btnSelect.selected == YES){
        
        [cell.btnSelect setImage:[UIImage imageNamed:@"btn_selected@3x.png"] forState:UIControlStateNormal];
        
    }else{
        
        [cell.btnSelect setImage:[UIImage imageNamed:@"btn_circle@3x.png"] forState:UIControlStateNormal];
    }
...
//    [nav.navigationBar setBackgroundImage:[self imageWithColor:kRGB(19, 153, 231)] forBarMetrics:UIBarMetricsDefault];
    ...
    [self.sender presentViewController:nav animated:YES completion:nil];
//    [self.sender presentVC:nav animated:YES completion:nil];
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self showCell:cell andIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath{
    NSLog(@"%@",@(indexPath.row));
   [self.showCellArr removeObject:cell];
//    [self showCellArrAddIndexPath:indexPath];
}

- (void)showCell:(UICollectionViewCell *)cell andIndexPath:(NSIndexPath *)indexPath{

    
    if (self.showCellArr.count == 0) {
        [self.showCellArr addObject:cell];
        [self setCellShow];
    }else{
        UICollectionViewCell *lastCell = [self.showCellArr lastObject];
        NSIndexPath *lastIndex = [self.collectionView indexPathForCell:lastCell];
        
        if (indexPath.row > lastIndex.row) {
            [self.showCellArr addObject:cell];
        }else{
            [self.showCellArr insertObject:cell atIndex:0];
        }
    }
}
...
- (void)setCellBtnPosition:(NSIndexPath *)indexPath{
    

    
    ZLCollectionCell *cell = (ZLCollectionCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    
    CGRect cellScreenFrame = [self.collectionView convertRect:cell.frame toView:self.baseView];
    
    CGRect cellScreenFrame1 = [self.baseView convertRect:cell.frame fromView:self.collectionView];
    
    cellScreenFrame.size = [self collectionView:self.collectionView layout:self.collectionView.collectionViewLayout sizeForItemAtIndexPath:indexPath];
    
//    if (cellScreenFrame.origin.x) {
//        <#statements#>
//    }
    
    NSLog(@"(%f,%f,%f,%f)",cellScreenFrame.origin.x,cellScreenFrame.origin.y,cellScreenFrame.size.width,cellScreenFrame.size.height);
    CGFloat cellX = cellScreenFrame.origin.x;
    CGFloat cellW = cellScreenFrame.size.width;
    CGFloat scrW = [[UIScreen mainScreen] bounds].size.width;
    
    if (cellX + cellW > scrW) {
        cell.selectTrailing.constant = cellX + cellW - scrW + 3;
    }else{
        cell.selectTrailing.constant = 3;
    }
    
//    NSInteger maxIndex = [self collectionView:self.collectionView numberOfItemsInSection:0] - 1;
//    
//    NSInteger willIndex = self.willIndexPath.row;
//    
//    if (self.isLeft) {
//        
//        
...
- (void)setCellShow{
    ZLCollectionCell *leftCell = [self.showCellArr firstObject];
    ZLCollectionCell *rightCell = [self.showCellArr lastObject];
    
    NSIndexPath *leftIndex = [self.collectionView indexPathForCell:leftCell];
    NSIndexPath *rightIndex = [self.collectionView indexPathForCell:rightCell];
    
    
    [self setCellBtnPosition:leftIndex];
    [self setCellBtnPosition:rightIndex];
...
//    CGFloat minOffset = curOffset - self.lastOffset;
//    if (minOffset > 0 ) {
//        self.isLeft = NO;
//    }else{
//        self.isLeft = YES;
...
////        NSInteger maxIndex = [self collectionView:self.collectionView numberOfItemsInSection:0] - 1;
////        NSInteger  willIndex = self.willIndexPath.row;
////        willIndex = willIndex + 1 > maxIndex ? maxIndex : willIndex + 1;
////            self.willIndexPath = [NSIndexPath indexPathForRow:willIndex inSection:0];
////    }
//    
//    if (self.willIndexPath) {
//        
//        [self setWillCellForIndexPath:self.willIndexPath];
//    }
}
